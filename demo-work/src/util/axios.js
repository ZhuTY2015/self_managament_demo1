 ...
 Axios.interceptors.request.use(config => {
   ...
   const token = sessionStorage.getItem('userInfo')?JSON.parse(sessionStorage.getItem('userInfo')).token:'';//这里取token之前，你肯定需要先拿到token,存一下
   if(token){
     // config.params = {'token':token} //如果要求携带在参数中
     /*for ( name in obj ) {

       config.params[name] = obj[name];

     }*/
     config.headers["timestamp"] = Date.parse(new Date());
     config.headers["nonce"] = parseInt(Math.random() * 100000000000, 10) + 1;
